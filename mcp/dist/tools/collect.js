...
export async function runCollect(args) {
    ...
    const tgTask = args.skip_telegram || !telegramActivated()
        ? Promise.resolve(null)
    ...
    let tgAdded = 0;
    if (tg && "jobs" in tg && Array.isArray(tg.jobs)) {
        const jobs = tg.jobs;
        if (jobs.length)
            upsertJobs(jobs);
        tgAdded = jobs.length;
    ...
    return {
        ...
        http: http
            ? "jobs" in http
                ? {
                    stored: http.jobs?.length ?? 0,
                    errors: http.errors?.slice(0, 8) ?? [],
                    served_from_cache: http.cached ?? [],
                }
                : { error: http.error }
            : "skipped",
        telegram: tg
...
export async function runDbSearch(args) {
    ...
    const query = args.query ??
        (args.keywords?.length
            ...
            : profile.keywords.length
                ...
                : "");
    const cutoffMs = args.hours != null
    ...
    const corpus = listJobs().filter((j) => {
        if (new Date(j.date).getTime() < cutoffMs)
            return false;
        if (platforms && !platforms.has(j.platform))
            return false;
        return true;
    });
