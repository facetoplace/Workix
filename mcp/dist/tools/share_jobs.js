 ...
 */
import { getJob, isHubShared, logOutreach, markHubShared, listHubShares, listUnsharedJobs, } from "../store.js";
...
function hubPublicUrl(sid, path) {
...
function recordShareResult(jobs, rows, status) {
    const at = new Date().toISOString();
    for (const row of rows) {
        const idx = typeof row.index === "number" ? row.index : -1;
        const job = (idx >= 0 && jobs[idx]) ||
            jobs.find((j) => j.link.replace(/\/$/, "") === String(row.externalUrl || row.url || "").replace(/\/$/, ""));
        if (!job || !row.sid)
            continue;
...
export async function runShareJobs(args) {
    ...
    for (const j of jobs) {
        ...
        if (!args.force && isHubShared(j.id)) {
            ...
            if (hs)
                already.push({ jobId: j.id, sid: hs.sid, hubUrl: hs.hubUrl });
            continue;
        }
        toSend.push(j);
        if (toSend.length >= 20)
    ...
    recordShareResult(toSend, (data.skipped || []).map((s) => ({
        ...s,
        externalUrl: s.url,
    })), "exists");
    return {
