...
import { contactedKeys, markDigestShown, normalizeLink, wasShownInDigest, } from "../store.js";
...
export async function runDigest(args) {
    ...
    return {
        ...
        ...(dropped_duplicates
            ? {
                dropped_duplicates,
                duplicates_note: "Схлопнуты дубли одной вакансии по городам (один и тот же постинг под разными city-слагами).",
            }
            : {}),
        ...
        ...(dropped_contacted
            ? {
                dropped_contacted,
                dropped_note: "Скрыто как уже отработанное (outreach). include_contacted:true — показать.",
            }
            : {}),
        filters: {
            keywords,
