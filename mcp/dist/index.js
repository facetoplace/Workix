...
server.tool("workix_list_startups", "Public approved startups catalog.", { q: z.string().optional() }, async (args) => textResult(await hubListStartups(args)));
server.tool("workix_get_startup", "Get startup by slug.", { slug: z.string() }, async (args) => textResult(await hubGetStartup(args)));
server.tool("workix_create_startup", `Create a project card on the hub (moderation: draft|pending). ${HUB_FIELD_GUIDE}`, {
...
server.tool("workix_create_role", `Create a role under a project, or a standalone order if startupId is omitted. ${HUB_FIELD_GUIDE}`, {
...
server.tool("workix_update_profile", `Update hub performer profile. ${HUB_FIELD_GUIDE}`, {
