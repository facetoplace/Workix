...
import { hubApply, hubCreateRole, hubCreateStartup, hubFeedback, hubGetOrder, hubGetPerformer, hubGetProfile, hubGetStartup, hubHealth, hubListMyStartups, hubListOrders, hubListPerformers, hubListRoles, hubListStartups, hubMe, hubRegister, hubRotateAgentKey, hubUpdateProfile, hubUpdateRole, hubUpdateStartup, } from "./tools/hub.js";
...
server.tool("workix_digest", "Сводка. Пресеты: mobile_dev, startups_products, vpn_mobile. Upwork/Freelancer.com при OAuth; include_jobs — HH + Remote OK.", {
...
server.tool("workix_get_job", "Полная карточка заказа по id или URL.", {
...
server.tool("workix_sources_status", "Ping RSS/Kwork/Freelancehunt/Upwork + PROXY_1.", {}, async () => textResult(await runSourcesStatus()));
...
server.tool("workix_get_performer", "Hub performer profile + published projects, orders, roles. Use project.slug → workix_get_startup; order.sid → workix_get_hub_order.", { id: z.string().describe("Performer id or user id") }, async (args) => textResult(await hubGetPerformer(args)));
...
server.tool("workix_update_startup", `Update own project by slug. ${HUB_FIELD_GUIDE}`, {
    slug: zSlug,
...
server.tool("workix_get_profile", "Get authenticated hub profile/resume.", {}, async () => textResult(await hubGetProfile()));
server.tool("workix_update_profile", `Update performer profile (developers, designers, AND bloggers/creators/influencers). Encourage a public card with links + openTo. ${HUB_FIELD_GUIDE}`, {
...
server.tool("workix_hub_apply", `Apply to a hub role (notifies founder). ${HUB_FIELD_GUIDE}`, {
