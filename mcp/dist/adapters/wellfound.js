...
function extractWf() {
    ...
    document.querySelectorAll('a[href*="/jobs/"]').forEach((a) => {
        ...
        const company = card?.querySelector('a[href*="/company/"]')?.textContent?.trim() ||
            card?.querySelector("[class*='company'], h2, h3")?.textContent?.trim() ||
            "";
...
export async function fetchWellfoundJobs(opts) {
    ...
    const { data, error } = await withProfilePage("wellfound", url, extractWf, {
        waitMs: 7000,
        scrolls: 5,
        headful: true,
    });
    if (error)
        return { jobs: [], error: `wellfound: ${error}` };
