...
export function careerjetConfigured() {
...
export async function fetchCareerjetJobs(opts) {
    ...
    const keywords = opts?.keywords?.trim() ||
        ...
        "developer";
    const location = opts?.location?.trim() || process.env.CAREERJET_LOCATION?.trim() || "";
    const locale = opts?.locale?.trim() || process.env.CAREERJET_LOCALE?.trim() || "en_GB";
    const pageSize = Math.min(Math.max(opts?.pageSize ?? 50, 1), 99);
    const referer = process.env.CAREERJET_REFERER?.trim() || "https://workix.co/";
    ...
    const { data, error, status } = await fetchJson(`https://public.api.careerjet.net/search?${qs}`, {
        headers: {
    ...
    if (data.error)
        return { jobs: [], error: `careerjet: ${data.error}` };
    const jobs = [];
