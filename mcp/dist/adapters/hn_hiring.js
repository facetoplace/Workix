...
async function latestThread() {
    const { data, error, status } = await fetchJson(`${ALGOLIA}/search_by_date?tags=story,author_whoishiring&hitsPerPage=20`, { headers: { Accept: "application/json" }, proxy: false });
    if (error || !data?.hits) {
...
export async function fetchHnHiringJobs(opts) {
    ...
    for (let page = 0; page < 6 && jobs.length < limit; page++) {
        const { data, error, status } = await fetchJson(`${ALGOLIA}/search?tags=comment,story_${storyId}&hitsPerPage=100&page=${page}`, { headers: { Accept: "application/json" }, proxy: false });
        if (error || !data?.hits) {
            // A first-page failure is the whole source; later pages just cap depth.
            if (page === 0)
                return { jobs: [], error: error || `hn algolia HTTP ${status}` };
            break;
        }
        ...
        if (data.nbPages != null && page + 1 >= data.nbPages)
            break;
    }
