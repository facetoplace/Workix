...
const HN_SHOW = "https://hn.algolia.com/api/v1/search_by_date?tags=show_hn&hitsPerPage=";
const REDDIT_SIDEPROJECT = "https://www.reddit.com/r/SideProject/new/.rss?limit=50";
function stripShowPrefix(title) {
    return title.replace(/^show\s+hn:\s*/i, "").trim();
}
async function fetchShowHn(limit) {
    const { data, error, status } = await fetchJson(`${HN_SHOW}${limit}`, { headers: { Accept: "application/json" }, proxy: false });
    if (error || !data?.hits) {
...
async function fetchSideProject(limit) {
...
export async function fetchLaunchLeads(opts) {
    ...
    const errors = [hn.error, reddit.error].filter(Boolean);
    const byLink = new Map();
    for (const j of [...hn.jobs, ...reddit.jobs])
        byLink.set(j.link, j);
    let jobs = [...byLink.values()].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
...
export async function pingLaunches() {
