...
function titleFrom(html, link) {
    const heading = html.match(/<h1[^>]*>([\s\S]*?)<\/h1>/i)?.[1]
        || html.match(/<h2[^>]*>([\s\S]*?)<\/h2>/i)?.[1]
        || html.match(/<title[^>]*>([\s\S]*?)<\/title>/i)?.[1];
    return decode(heading || link.split("/").pop() || "Remocate role").slice(0, 200);
}
export async function fetchRemocateJobs(opts) {
    const limit = Math.min(Math.max(opts?.limit ?? 40, 1), 100);
    const pages = Math.min(Math.max(opts?.pages ?? 3, 1), 10);
    const pagesFetched = await Promise.all(Array.from({ length: pages }, (_, i) => fetchText(`${BASE}/${i ? `?ee7bb4b9_page=${i + 1}` : ""}`, { proxy: false, timeoutMs: 20000 })));
    ...
    const jobs = (await Promise.all(urls.map(async (link) => {
