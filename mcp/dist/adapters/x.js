...
function extractX() {
    ...
    cards.forEach((card) => {
        const timeA = card.querySelector("a[href*='/status/'] time")?.parentElement;
        ...
        if (seen.has(link))
            return;
        seen.add(link);
        const text = (card.querySelector("[data-testid='tweetText']")?.textContent || "")
            .replace(/\s+/g, " ")
            .trim();
        if (!text || text.length < 8)
            return;
        const nameBlock = (card.querySelector("[data-testid='User-Name']")?.textContent || "")
            .replace(/\s+/g, " ")
            .trim();
...
function parseQueries() {
    ...
    for (const part of raw.split(";")) {
        ...
        if (rest.length)
            out.push({ category: label.trim() || "custom", query: rest.join("::").trim() });
        else
            out.push({ category: "custom", query: seg });
    }
...
export async function fetchXLeads(opts) {
    if (process.env.X_ENABLE !== "1") {
        return {
            jobs: [],
            error: "x: ToS forbids automated collection — set X_ENABLE=1 to opt in to browser ingest (optional)",
        };
    }
    ...
    for (const lane of lanes) {
        if (jobs.length >= limit)
            break;
        const { data, error } = await withProfilePage("x", lane.url, extractX, { waitMs: 7000, scrolls: 4, headful: true });
        if (error) {
            errors.push(error);
            continue;
        }
    ...
    if (!jobs.length && !errors.length) {
        return {
            jobs: [],
            error: "x: no posts rendered — session may be logged out or blocked (re-run board-open.mjs x https://x.com/home)",
        };
    }
