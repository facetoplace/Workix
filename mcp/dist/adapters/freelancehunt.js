...
export function freelancehuntConfigured() {
    loadEnv();
    return Boolean(process.env.FREELANCEHUNT_TOKEN?.trim());
}
export async function fetchFreelancehuntJobs() {
    loadEnv();
    ...
    for (const p of data.data) {
        ...
        const link = p.links?.self?.web || `https://freelancehunt.com/project/${p.id}.html`;
        if (!title || !link)
            continue;
...
export async function freelancehuntBid(opts) {
    ...
    const token = process.env.FREELANCEHUNT_TOKEN?.trim();
    if (!token)
        return { ok: false, error: "FREELANCEHUNT_TOKEN missing" };
