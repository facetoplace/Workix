...
export function freelancerConfigured() {
    loadEnv();
...
async function flnFetch(path, opts) {
    ...
    if (!token)
        return { error: "FREELANCER_OAUTH_TOKEN missing", status: 0 };
    try {
...
export async function fetchFreelancerJobs(opts) {
    ...
    if (r.error && !r.data?.projects) {
        ...
        const r2 = await flnFetch(`/projects/0.1/projects/?${alt}`);
...
export async function freelancerPlaceBid(opts) {
    ...
    if (r.error)
        return { ok: false, error: r.error, raw: r.data };
    return { ok: true, raw: r.data };
}
export function freelancerProjectId(job) {
    const raw = job.raw;
