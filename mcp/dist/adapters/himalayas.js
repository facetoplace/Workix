...
function formatSalary(row) {
    ...
    if (row.minSalary != null && row.maxSalary != null) {
        if (row.minSalary === row.maxSalary)
            return `${row.minSalary} ${cur}${period}`;
        return `${row.minSalary}–${row.maxSalary} ${cur}${period}`;
    }
    if (row.minSalary != null)
        return `from ${row.minSalary} ${cur}${period}`;
    if (row.maxSalary != null)
        return `up to ${row.maxSalary} ${cur}${period}`;
    return undefined;
}
export async function fetchHimalayasJobs(opts) {
    ...
    for (let page = 0; page < maxPages; page++) {
        ...
        if (total && offset + data.jobs.length >= total)
            break;
    }
