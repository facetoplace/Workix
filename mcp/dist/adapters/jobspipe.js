...
export function jobspipeKey() {
    // JOBS_PIPE_KEY is the spelling on their dashboard; both are accepted.
    return (process.env.JOBSPIPE_API_KEY?.trim() ||
        process.env.JOBS_PIPE_KEY?.trim() ||
        "");
}
export function jobspipeConfigured() {
...
export function resetJobspipeUsage() {
    const fresh = {
        month: currentMonth(),
        jobs: 0,
        calls: 0,
...
function buildBody(f) {
    ...
    if (excludeTitles?.length)
        body.job_title_not = excludeTitles;
...
export async function fetchJobsPipeJobs(filters) {
    ...
    if (remaining <= 0) {
        return {
            jobs: [],
            error: `jobspipe: monthly budget spent (${used}/${budget} jobs this month). ` +
    ...
    for (const j of rows) {
        ...
        const budgetStr = j.salary_string ||
        ...
        jobs.push({
            ...
            date: (j.date_posted || j.discovered_at)
                ? new Date((j.date_posted || j.discovered_at)).toISOString()
...
export async function detectCompanyTechStack(args) {
    ...
    try {
        ...
        if (!text)
            return { ok: false, error: "empty tech stack response" };
        try {
