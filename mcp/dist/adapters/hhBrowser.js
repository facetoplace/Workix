...
export async function closeHhBrowser() {
    if (!cached)
        return;
    try {
        if (cached.owned)
...
export async function fetchHhHtmlViaBrowser(url) {
    const b = await getBrowser();
    if (!b) {
        return {
            error: "puppeteer-core/Chrome недоступны — cd mcp && npm install puppeteer-core",
        };
    }
    let page;
    try {
