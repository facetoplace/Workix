...
function jsonLd(html) {
    for (const raw of html.matchAll(/<script[^>]+type=["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/gi)) {
        try {
            ...
            if (value["@type"] === "JobPosting" || value.title || value.description)
                return value;
        }
...
function links(html) {
    const out = new Set();
    for (const m of html.matchAll(/href=["'](\/jobs\/[a-z0-9]+)["']/gi))
        out.add(`${BASE}${m[1]}`);
    return [...out];
}
export async function fetchTheHubJobs(opts) {
    ...
    const jobs = details.filter((job) => Boolean(job));
