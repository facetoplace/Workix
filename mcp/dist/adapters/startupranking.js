...
export function parseRankingRows(html) {
    const rows = [];
    const chunks = html.match(/<tr id="startup_[\s\S]*?<\/tr>/gi) || [];
    for (const chunk of chunks) {
        const linkM = chunk.match(/<div class="name">\s*<a href="\/startup\/([^"]+)"[^>]*>([\s\S]*?)<\/a>/i);
        if (!linkM)
            continue;
        const slug = linkM[1];
        ...
        const descM = chunk.match(/<td class="tleft description">([\s\S]*?)<\/td>/i);
 ...
 */
async function getRanking(path, attempts) {
    let status = 0;
...
export async function pingStartupRanking() {
