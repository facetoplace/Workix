...
function apiKey() {
    return (process.env.SUPERTEAM_EARN_API_KEY?.trim() ||
        process.env.SUPERTEAM_EARN_AGENT_KEY?.trim() ||
        "");
}
function baseUrl() {
    return (process.env.SUPERTEAM_EARN_BASE?.trim().replace(/\/$/, "") ||
        "https://superteam.fun");
}
export function superteamEarnConfigured() {
    return Boolean(apiKey());
}
function formatReward(row) {
    const token = row.token || "USDC";
    if (row.rewardAmount != null)
        return `${row.rewardAmount} ${token}`;
...
export async function fetchSuperteamEarnJobs(opts) {
    ...
    if (!key) {
        return {
            jobs: [],
            error: "optional: missing SUPERTEAM_EARN_API_KEY (POST /api/agents → apiKey)",
        };
    }
    const take = Math.min(Math.max(opts?.take ?? 40, 1), 100);
    const qs = new URLSearchParams({ take: String(take) });
    const type = opts?.type?.trim() || process.env.SUPERTEAM_EARN_TYPE?.trim() || "";
    if (type)
        qs.set("type", type);
