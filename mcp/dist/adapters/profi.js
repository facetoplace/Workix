 ...
 */
function extractProfi() {
    const seen = new Set();
    const items = [];
    const anchors = document.querySelectorAll("a[data-testid$='order-snippet'], a[href*='/backoffice/n.php?o=']");
    anchors.forEach((a) => {
        ...
        if (seen.has(link))
            return;
        seen.add(link);
        const clean = (s) => (s || "").replace(/\bfalse\b/g, " ").replace(/\s+/g, " ").trim();
...
export async function fetchProfiJobs(opts) {
    ...
    const { data, error } = await withProfilePage("profi", url, extractProfi, { waitMs: 6000, scrolls: 3 });
    if (error)
        return { jobs: [], error: `profi: ${error}` };
    const rows = data?.items || [];
    if (!rows.length) {
        return {
            jobs: [],
            error: "profi: no order cards rendered — session may be logged out (re-run board-open.mjs profi https://profi.ru/backoffice/)",
        };
    }
