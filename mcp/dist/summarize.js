...
export function cardSummary(job, keywords) {
...
export function digestText(cards, meta) {
    const byPlatform = {};
    ...
    const lines = [
        ...
        `По площадкам: ${Object.entries(byPlatform)
            .map(([k, v]) => `${k}=${v}`)
            .join(", ") || "—"}`,
    ];
    ...
    cards.slice(0, 15).forEach((c, i) => {
        lines.push(`${i + 1}. [${c.platform}${c.kind ? "/" + c.kind : ""}] ${c.title}${c.budget ? ` · ${c.budget}` : ""}`);
        lines.push(`   ${c.link}`);
