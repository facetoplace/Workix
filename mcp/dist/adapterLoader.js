...
function trustedAdapterHosts(registryBaseUrl) {
    const hosts = new Set(["workix.co", "www.workix.co"]);
    if (registryBaseUrl) {
        try {
 ...
 */
function assertTrustedDownloadUrl(url, registryBaseUrl) {
    if (process.env.WORKIX_MCP_ALLOW_UNTRUSTED_REGISTRY === "1")
        return;
    if (!/^https?:\/\//i.test(url)) {
    ...
    if (!hosts.has(u.hostname)) {
        throw new Error(`untrusted adapter host: ${u.hostname} (set WORKIX_MCP_TRUSTED_HOSTS or WORKIX_MCP_ALLOW_UNTRUSTED_REGISTRY=1)`);
    }
}
