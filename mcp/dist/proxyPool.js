...
function isUsableProxy(line) {
    return (/^(socks5?|https?):\/\//i.test(line) ||
...
function parseProxyLines(text) {
    ...
    for (const line of body.split(/\r?\n/)) {
        ...
        for (const part of t.split(/[,;|]+/).map((x) => x.trim())) {
            if (isUsableProxy(part))
                out.push(normalizeProxy(part));
        }
    }
    return [...new Set(out)];
}
 ...
 */
export async function getProxyPool() {
    loadEnv();
...
export async function nextProxy() {
...
export async function proxyPoolInfo() {
    const pool = await getProxyPool();
    const protocols = [
        ...new Set(pool.map((u) => (u.match(/^([a-z0-9]+):/i) || ["", "?"])[1].toLowerCase())),
    ];
    return {
        count: pool.length,
        protocols,
