...
function rows(payload: unknown): StartupJobRow[] {
  if (Array.isArray(payload)) return payload as StartupJobRow[];
  if (!payload || typeof payload !== "object") return [];
  const p = payload as Record<string, unknown>;
  for (const key of ["jobs", "results", "data", "listings"]) {
    if (Array.isArray(p[key])) return p[key] as StartupJobRow[];
  }
  return [];
}

export function startupJobsMcpEnabled(): boolean {
  return process.env.STARTUP_JOBS_MCP_ENABLED !== "0";
}

export async function callStartupJobsMcp(tool: string, args: Record<string, unknown> = {}) {
  ...
  if (!allowed.has(tool)) return { error: `startup_jobs MCP tool is not allowlisted: ${tool}` };
  if (!startupJobsMcpEnabled()) return { error: "startup_jobs MCP disabled" };
  return callMcpTool({
...
export async function fetchStartupJobsMcp(opts?: {
  ...
  if (!startupJobsMcpEnabled()) return { jobs: [], error: "startup_jobs MCP disabled" };
  ...
  if (res.error || res.isError) return { jobs: [], error: res.error || `startup_jobs MCP: ${(res.text || "request rejected").slice(0, 300)}` };
