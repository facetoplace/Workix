...
const FUNDING_RE =
  /\b(raise[sd]?|raising|secure[sd]?|land[sd]?|nab[sd]?|clos(?:es|ed)\s+(?:a\s+)?(?:\$|€|£|round)|bag[sd]?|fundrais\w*|funding\s+round|series\s+[a-h]\b|seed\s+round|pre-seed|valuation)\b|привлек\w*|подня\w+\s+раунд/i;

const AMOUNT_RE =
  /([$€£]\s?\d[\d.,]*\s*(?:m|bn|k|million|billion)?|\d[\d.,]*\s*(?:млн|млрд)\s*(?:\$|₽|руб\w*|евро)?)/i;
...
const COMPANY_RE =
  /^(.{2,80}?)\s+(?:raise[sd]?|raising|secure[sd]?|land[sd]?|nab[sd]?|clos(?:es|ed)|bag[sd]?|hits?|snaps?)\b/i;

function feedsFromEnv(): Feed[] {
...
export function looksLikeFunding(title: string): boolean {
...
const NOT_A_COMPANY =
  /\b(startups?|spinouts?|funding|rounds?|investors?|looking|deals?|week|month|europe|european)\b/i;
...
function stripLeadIn(name: string): string {
  let out = name.trim();
  for (let i = 0; i < 3; i++) {
    const before = out;
...
export function parseHeadline(title: string): {
  company?: string;
  amount?: string;
  stage?: string;
} {
...
function dedupeByCompany(jobs: Job[]): Job[] {
  ...
  const out: Job[] = [];
  for (const j of jobs) {
    const company = (j.raw as { company?: string }).company;
    if (!company) {
      out.push(j);
      continue;
    }
    ...
    const alsoIn = (prev.raw as { alsoReportedBy?: string[] }).alsoReportedBy || [];
    (prev.raw as { alsoReportedBy?: string[] }).alsoReportedBy = [
      ...alsoIn,
      (j.raw as { feed?: string }).feed || "?",
    ];
  }
  return out;
}

function toJob(feed: Feed, item: Parser.Item): Job | null {
  const link = item.link?.trim();
  const title = clean(item.title || "");
  if (!link || !link.startsWith("http") || !title) return null;
  if (!looksLikeFunding(title) || FUND_NEWS.test(title)) return null;
...
export async function fetchFundingLeads(opts?: {
  limit?: number;
  keywords?: string[];
}): Promise<{ jobs: Job[]; error?: string; feeds?: number }> {
    ...
    feeds.map(async (feed) => {
      ...
      try {
        ...
        for (const item of parsed.items || []) {
          ...
          if (job) byLink.set(job.link, job);
        }
      } catch (e) {
        errors.push(
          `${feed.id}: ${e instanceof Error ? e.message : String(e)}`,
        );
      }
    }),
  );

  let jobs = dedupeByCompany(
    [...byLink.values()].sort(
...
export async function pingFunding(): Promise<{
  platform: string;
  ok: boolean;
  status: number;
  ms: number;
