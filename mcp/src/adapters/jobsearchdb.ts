...
export async function fetchJobSearchDbBoards(opts?: { keywords?: string[]; limit?: number }): Promise<{ jobs: Job[]; error?: string }> {
  const r = await fetchText(BASE, { proxy: false, timeoutMs: 20000 });
  if (!r.ok) return { jobs: [], error: `jobsearchdb: public directory unavailable (${r.status || r.error || "network error"})` };
  ...
  for (const m of r.text.matchAll(/href=["'](https?:\/\/[^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)) {
    ...
    if (/jobsearchdb\.com/i.test(new URL(url).hostname)) continue;
    const label = decode(m[2]) || url;
    if (/sponsor|linkedin|submit|newsletter|facebook|twitter|instagram/i.test(label)) continue;
    links.set(url, label);
  }
