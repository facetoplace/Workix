...
interface GmOffer {
  ...
  salary_taxes?: string | null;
  english_level?: string | null;
  skills_objects?: Array<{ name?: string } | string>;
  location_items?: Array<{ label?: string; format?: string }>;
  company?: { id?: number | null; name?: string };
}

function skillNames(raw: GmOffer["skills_objects"]): string[] {
...
function salaryOf(o: GmOffer): string | undefined {
  if (o.salary_hidden) return undefined;
  // The board already formats this the way it shows on the card
  // ("5 000 — 8 000 $/мес на руки"), so prefer it over recomposing.
  if (o.salary_description) return o.salary_description.replace(/​/g, "").trim();
...
export async function fetchGetmatchJobs(opts?: {
  ...
  const want = Math.min(
    Math.max(opts?.limit ?? (Number(process.env.GETMATCH_LIMIT) || 100), 1),
    750,
  );
