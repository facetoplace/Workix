...
type Board = { id: string; base: string; paths: string[]; host: RegExp; cookieJar?: string };
...
function jsonLd(html: string): Record<string, unknown>[] {
  ...
  for (const m of html.matchAll(/<script[^>]+type=["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/gi)) {
    try {
      const value = JSON.parse(m[1]) as Record<string, unknown>;
      if (Array.isArray(value.itemListElement)) out.push(...value.itemListElement.filter((x): x is Record<string, unknown> => Boolean(x && typeof x === "object")));
      else out.push(value);
...
function links(board: Board, html: string): string[] {
  const out = new Set<string>();
  for (const m of html.matchAll(/href=["']([^"']+)["']/gi)) {
    try {
      const url = new URL(m[1], board.base);
      if (board.host.test(url.hostname) && /job|offer|vacanc|position|career|posting/i.test(url.pathname) && url.pathname !== "/vacancies") out.add(url.toString());
    } catch { /* ignore malformed links */ }
  }
  return [...out];
}

function field(data: Record<string, unknown> | undefined, ...keys: string[]): string | undefined {
  for (const key of keys) if (data?.[key]) return String(data[key]);
  return undefined;
}

export async function fetchRegionalBoardJobs(platform: string, opts?: { keywords?: string[]; limit?: number }): Promise<{ jobs: Job[]; error?: string }> {
  ...
  if (!board) return { jobs: [], error: `${platform}: unsupported regional board` };
  ...
  if (!html) return { jobs: [], error: `${platform}: public page unavailable` };
  ...
  for (const item of structured) {
    const nested = (item.item as Record<string, unknown> | undefined) || item;
    ...
    if (words.length && !words.some((w) => `${title} ${description}`.toLowerCase().includes(w))) continue;
  ...
  if (jobs.length) return { jobs: jobs.slice(0, limit) };
  ...
  const out = details.filter((j): j is Job => Boolean(j));
