...
function extractYc(): { url: string; items: RawYc[] } {
  ...
  document.querySelectorAll('a[href*="/jobs/"]').forEach((a) => {
    ...
    const company =
      card?.querySelector('a[href*="/companies/"]')?.textContent?.trim() ||
      (card?.querySelector("[class*='company']") as HTMLElement | null)?.textContent?.trim() ||
      "";
...
export async function fetchYcJobs(opts?: {
  ...
  const { data, error } = await withProfilePage<{ url: string; items: RawYc[] }>("yc", url, extractYc, {
    waitMs: 6000,
  });
  if (error) return { jobs: [], error: `yc: ${error}` };
