...
function matches(post: PhPost, keywords?: string[]): boolean {
...
function toJob(post: PhPost): Job | undefined {
  if (!post.name || !post.url) return undefined;
...
async function query(
  first: number,
): Promise<{ posts: PhPost[]; error?: string; status: number }> {
  ...
  try {
    ...
    if (!res.ok) {
      return {
        posts: [],
        status: res.status,
        error: `producthunt: HTTP ${res.status}${
...
export async function fetchProductHuntLeads(opts?: {
  ...
  const first = Math.min(
    Math.max(opts?.limit ?? envLimit(), 1),
    MAX_LIMIT,
  );

  // No key is not a failure here — the public feed covers the same launches.
  if (!productHuntConfigured()) return fetchFeed(first, opts?.keywords);
  ...
  for (const post of r.posts) {
    if (!matches(post, opts?.keywords)) continue;
...
export async function pingProductHunt(): Promise<{
  platform: string;
  ok: boolean;
  status: number;
  ms: number;
