 ...
 */
function extractAvito(): { url: string; items: RawAvito[] } {
  const seen = new Set<string>();
  const items: RawAvito[] = [];
  const cards = document.querySelectorAll("[data-marker='item']");
  const nodes: Element[] = cards.length
    ? Array.from(cards)
    : Array.from(document.querySelectorAll("a[href*='/vakansii/']"));
  nodes.forEach((node) => {
    const card = (node.closest("[data-marker='item']") as HTMLElement | null) || (node as HTMLElement);
    const titleA =
      (card.querySelector("[data-marker='item-title']") as HTMLAnchorElement | null) ||
      (card.matches("a[href]") ? (card as HTMLAnchorElement) : null) ||
      (card.querySelector("a[href*='/vakansii/']") as HTMLAnchorElement | null);
    if (!titleA) return;
...
export async function fetchAvitoJobs(opts?: {
  ...
  if (process.env.AVITO_ENABLE !== "1") {
    return {
      jobs: [],
      error:
        "avito: ToS restricts automated collection — set AVITO_ENABLE=1 to opt in to browser ingest (optional)",
    };
  }
  const url = process.env.AVITO_URL || "https://www.avito.ru/all/vakansii";
  const { data, error } = await withProfilePage<{ url: string; items: RawAvito[] }>(
    "avito",
    url,
    extractAvito,
    { waitMs: 7000, scrolls: 3, headful: true },
  );
  if (error) return { jobs: [], error: `avito: ${error}` };
  const rows = data?.items || [];
  if (!rows.length) {
    return {
      jobs: [],
      error:
        "avito: no cards rendered — session may be logged out or blocked (re-run board-open.mjs avito)",
    };
  }
