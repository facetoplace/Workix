...
function title(html: string, fallback: string): string {
...
function links(html: string, kind: "project" | "profile"): string[] {
  const out = new Set<string>();
  const re = new RegExp(`href=["'](\\/${kind}\\/[^"'#?]+)`, "gi");
  for (const m of html.matchAll(re)) out.add(`${BASE}${m[1]}`);
  return [...out];
}
...
export async function fetchStartupiumLeads(opts?: {
  ...
  for (const link of detailLinks) {
    ...
    if (kind === "projects" && !isProject) continue;
    if (kind === "profiles" && isProject) continue;
    ...
    if (words.length && !words.some((w) => `${name} ${desc}`.toLowerCase().includes(w))) continue;
