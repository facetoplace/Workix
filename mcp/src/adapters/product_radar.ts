...
function parseCards(html: string): Job[] {
  const jobs: Job[] = [];
  const cards =
    html.match(/<article class="products__item[\s\S]*?<\/article>/gi) || [];
  for (const card of cards) {
    const linkM =
      card.match(
        /class="card__title"[^>]*>[\s\S]*?href="(https?:\/\/productradar\.ru\/product\/[^"]+)"[^>]*>([\s\S]*?)<\/a>/i,
      ) ||
      card.match(
        /href="(https?:\/\/productradar\.ru\/product\/[^"#?]+)"[^>]*>\s*([^<]{2,120})\s*<\/a>/i,
      );
    if (!linkM) continue;
    let link = linkM[1].replace(/\/?$/, "/");
    const name = stripHtml(linkM[2]);
    if (!name || /#comments/i.test(link)) continue;

    const descM = card.match(
      /class="card__description"[^>]*>([\s\S]*?)<\/p>/i,
    );
    const hireM = card.match(
      /class="card__status-text"[^>]*>([\s\S]*?)<\/p>/i,
    );
    const desc = descM ? stripHtml(descM[1]) : "";
    const hiring = hireM ? stripHtml(hireM[1]) : "";
    if (!hiring && !desc) continue;
...
export async function fetchProductRadarJobs(opts?: {
  ...
  for (let page = 1; page <= maxPages; page++) {
    ...
    if (!batch.length) {
      if (page === 1) lastError = "no product cards in HTML";
      break;
    }
    pagesOk += 1;
    for (const j of batch) byLink.set(j.link, j);
  }
...
export async function pingProductRadar(): Promise<{
  platform: string;
  ok: boolean;
  status: number;
  ms: number;
