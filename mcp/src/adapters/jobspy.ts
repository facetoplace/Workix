...
const SITE_BY_PLATFORM: Record<string, string> = {
  indeed: "indeed",
  glassdoor: "glassdoor",
 ...
 */
const INSTALL_HINT =
  "JobSpy is not installed. It is optional: pip install -U python-jobspy — " +
 ...
 */
const STALE_HINT =
  "This usually means the credentials baked into your jobspy release were " +
  "rotated by the board. Upgrade to pick up the new ones: " +
  "pip install -U python-jobspy";

const STALE_SIGNALS =
  /\b(401|403|unauthorized|forbidden|invalid[_ -]?api[_ -]?key|authentication)\b/i;
...
const BUG_HINT =
  "That is an exception inside jobspy itself, not a block — upgrading or " +
  "retrying will not help. Check its issue tracker: " +
  "https://github.com/speedyapply/JobSpy/issues";

const BLOCKED_HINT =
  "The board refused the request. Usually an IP-level block (Cloudflare) or a " +
  "rate limit rather than anything wrong on your side — try again later, from " +
  "a different IP, or pass proxies.";

const SOCKS_HINT =
  "jobspy talks to the boards through `requests`, which needs PySocks before " +
...
function diagnose(stderr: string): string {
...
function pythonCandidates(env: NodeJS.ProcessEnv): string[] {
  return [env.PYTHON_BIN, "python3", "python"].filter(Boolean) as string[];
}
...
async function resolvePython(env: NodeJS.ProcessEnv): Promise<string | null> {
...
export async function jobspyAvailable(env: NodeJS.ProcessEnv): Promise<boolean> {
  return (await resolvePython(env)) !== null;
}

/** Installed python-jobspy version, or null when the bridge is unavailable. */
export async function jobspyStatus(env: NodeJS.ProcessEnv): Promise<{
  available: boolean;
  python?: string;
  version?: string;
  platforms: string[];
...
function money(row: JobSpyRow): string | undefined {
 ...
 */
function filterProxies(proxies?: string[]): string[] | undefined {
...
export async function fetchJobSpyJobs(opts: {
  ...
  const params = {
    ...
    linkedin_fetch_description:
      site === "linkedin" ? opts.env.JOBSPY_LINKEDIN_DESCRIPTIONS === "1" : undefined,
  };
  ...
  if (!res.ok) {
    ...
    return {
      jobs: [],
      error:
        `jobspy ${opts.platform} (v${cachedVersion || "?"}): ${tail || "failed"}.` +
        diagnose(res.stderr),
    };
  }
  ...
  let rows: JobSpyRow[];
  try {
    rows = JSON.parse(res.stdout.trim() || "[]") as JobSpyRow[];
  } catch {
  ...
  if (!rows.length) {
    ...
    if (logged.length) {
      return {
        jobs: [],
        error:
          `jobspy ${opts.platform} (v${cachedVersion || "?"}) returned nothing and ` +
    ...
    return {
      jobs: [],
      error:
        `jobspy ${opts.platform} (v${cachedVersion || "?"}) returned no rows and ` +
