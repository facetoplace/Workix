...
interface ClawTasksResponse {
  items?: ClawTask[];
  tab?: string;
  counts?: Record<string, number>;
}

function taskTitle(row: ClawTask): string | undefined {
  return row.metadata?.title || row.title || undefined;
}

function taskDesc(row: ClawTask): string {
  return (
...
export async function fetchClawEarnJobs(opts?: {
  ...
  const { data, error, status } = await fetchJson<ClawTasksResponse>(url, {
    headers: {
