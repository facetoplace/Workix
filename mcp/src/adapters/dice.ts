...
function postedWindow(hours?: number): "ONE" | "THREE" | "SEVEN" | undefined {
...
export async function fetchDiceJobs(opts?: {
  ...
  const keyword =
    (opts?.keywords || []).filter(Boolean).join(" ").trim() ||
    process.env.DICE_KEYWORD?.trim() ||
    "developer";
  const limit = Math.min(Math.max(opts?.limit ?? 50, 1), 100);
  const sponsor =
    opts?.willingToSponsor ?? (process.env.DICE_SPONSOR_ONLY === "1" || undefined);
  const remoteOnly =
    opts?.remoteOnly ?? (process.env.DICE_REMOTE_ONLY === "1" || undefined);
  ...
  const payload = (res.structured ?? res.data) as DicePayload | undefined;
