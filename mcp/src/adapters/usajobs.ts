...
/**
 * USAJOBS — the US federal job registry. Free API key from
 * https://developer.usajobs.gov/apirequest/ (email + key, no approval queue).
 *
 * Both headers are mandatory: the key alone gets a 403, because the API
 * identifies callers by the email in User-Agent.
 */
...
export async function fetchUsaJobs(opts?: {
  ...
  const keyword =
    opts?.keyword?.trim() || process.env.USAJOBS_KEYWORD?.trim() || "software";
  ...
  const { data, error, status } = await fetchJson<UsaJobsResponse>(
  ...
  for (const item of data.SearchResult.SearchResultItems) {
    ...
    if (!d?.PositionTitle || !link) continue;
