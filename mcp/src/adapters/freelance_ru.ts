...
function parseTaskCards(html: string): Job[] {
  const jobs: Job[] = [];
  const re =
    /<article class="task-card[^"]*"[\s\S]*?<\/article>/gi;
  const cards = html.match(re) || [];
  for (const card of cards) {
    const linkM = card.match(
      /<a[^>]*class="[^"]*task-card__title-link[^"]*"[^>]*href="(\/task\/view\/\d+)"[^>]*>([\s\S]*?)<\/a>/i,
    ) || card.match(
      /<a[^>]*href="(\/task\/view\/\d+)"[^>]*class="[^"]*task-card__title-link[^"]*"[^>]*>([\s\S]*?)<\/a>/i,
    );
    if (!linkM) continue;
    const path = linkM[1];
    const title = stripHtml(linkM[2]);
    if (!title) continue;
    const descM = card.match(
      /<p class="task-card__desc">([\s\S]*?)<\/p>/i,
    );
    const budgetM = card.match(
      /<div class="task-card__budget(?:\s[^"]*)?">([\s\S]*?)<\/div>/i,
    );
    const dateM = card.match(
      /task-card__foot-item"[^>]*title="([^"]+)"/i,
    );
    const catM = card.match(
      /task-chip--cat"[^>]*>([\s\S]*?)<\/span>/i,
    );
...
export async function fetchFreelanceRuJobs(): Promise<{
  jobs: Job[];
  error?: string;
  viaProxy?: boolean;
  source?: string;
  ...
  return {
    jobs: [],
    error:
      "Freelance.ru: HTML /task empty (нужен PROXY_1 с RU SOCKS5; RSS /rss/projects.xml обычно 404)",
  };
}

export async function pingFreelanceRu(): Promise<{
  platform: string;
  ok: boolean;
  status: number;
  ms: number;
