...
interface HhVacancy {
  ...
  published_at?: string;
  publicationTime?: { $?: string } | string;
  snippet?: { requirement?: string; responsibility?: string };
  salary?: { from?: number; to?: number; currency?: string };
  compensation?: { from?: number; to?: number; currencyCode?: string };
...
function extractInitialState(html: string): Record<string, unknown> | undefined {
  const m = html.match(
    /<template[^>]+id="HH-Lux-InitialState"[^>]*>([\s\S]*?)<\/template>/i,
  );
...
export async function fetchHhState(
  url: string,
): Promise<{ state?: Record<string, unknown>; status: number; error?: string; via?: string }> {
  if (!hasJar(JAR)) {
  ...
  if (res.ok && !isChallenge(res.text)) {
    ...
    if (state) return { state, status: res.status, via: "http" };
  }
...
export async function verifySession(): Promise<{
  ...
  const userType =
    ...
    ((state?.session as Record<string, unknown>)?.userType as string) ||
  ...
  const authorized =
    roleAuthorized || Boolean(userType && userType !== "anonymous");
...
function normalize(v: HhVacancy): Job | undefined {
  ...
  if (pay) {
    ...
    const cur =
      (pay as { currency?: string }).currency ||
      (pay as { currencyCode?: string }).currencyCode ||
      "RUR";
...
async function fetchViaSession(
  ...
  for (let page = 0; page < pages; page++) {
    const url =
      `https://hh.ru/search/vacancy?text=${encodeURIComponent(text)}` +
    ...
    const result = state?.vacancySearchResult as
      | { vacancies?: HhVacancy[] }
      | undefined;
    const items = result?.vacancies;
    if (!items?.length) {
      if (page === 0) lastError = "HH web: vacancySearchResult empty (вёрстка/сессия)";
      break;
    }
...
export async function fetchHhJobs(opts?: {
  ...
  const attempts: Array<() => Promise<{ jobs: Job[]; error?: string }>> = [];
  if (hasJar(JAR)) attempts.push(() => fetchViaSession(text, maxPages));
