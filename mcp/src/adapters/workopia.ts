...
function pickCards(payload: unknown): WorkopiaCard[] {
  ...
  for (const b of buckets) {
    ...
    return b.map((row) => {
      ...
      return (r.card && typeof r.card === "object" ? r.card : r) as WorkopiaCard;
    });
  }
  return [];
}

function companyOf(c: WorkopiaCard): string | undefined {
...
interface StoredTokens {
  ...
  /** Epoch ms; absent means "unknown, try it and refresh on failure". */
  expires_at?: number;
}
...
async function refresh(t: StoredTokens): Promise<StoredTokens | undefined> {
  if (!t.refresh_token) return undefined;
 ...
 * halfway through.
 */
export async function workopiaToken(): Promise<string | undefined> {
...
export function workopiaConfigured(): boolean {
 ...
 */
export async function workopiaLogin(): Promise<{
  ok: boolean;
  authorizeUrl?: string;
  error?: string;
}> {
  ...
  if (!clientId) return { ok: false, error: "register returned no client_id" };
  ...
  if (!code.ok) return { ok: false, authorizeUrl, error: code.error };
  ...
  if (!j.access_token) return { ok: false, error: "token response had no access_token" };
...
function waitForCode(
  state: string,
): Promise<{ ok: true; value: string } | { ok: false; error: string; value?: undefined }> {
  return new Promise((resolve) => {
    const server = http.createServer((req, res) => {
      ...
      server.close();
      if (!code) resolve({ ok: false, error: url.searchParams.get("error") || "no code in redirect" });
      // A mismatched state means the redirect did not come from our request.
      else if (got !== state) resolve({ ok: false, error: "state mismatch — redirect not ours" });
      else resolve({ ok: true, value: code });
    });
 ...
 */
export function resolveCity(explicit?: string): string | undefined {
  const direct = explicit?.trim() || process.env.WORKOPIA_CITY?.trim();
  if (direct) return direct;
  return cityFromProfile();
}

export function cityFromProfile(): string | undefined {
  let text: string;
  try {
    text = loadProfile();
  } catch {
...
function cleanCity(raw: string): string | undefined {
  ...
  if (/^[A-Z]{2}$/.test(first)) return undefined;
  return first;
}
...
export async function fetchWorkopiaJobs(opts?: {
  ...
  if (!token) {
    return {
      jobs: [],
      error:
        "workopia: optional — not signed in. There is no API key to copy: the " +
  ...
  if (!city) {
    return {
      jobs: [],
      error:
        "workopia: optional — no city to search. Its job_tool requires one, " +
  ...
  const jobTitle =
    (opts?.keywords || []).filter(Boolean).join(" ").trim() || "developer";
