...
server.tool(
  ...
  "Ping RSS (FL/Freelance/Weblancer/Habr)/Kwork/Freelancehunt/Upwork + PROXY_1.",
  {},
...
server.tool(
  "workix_get_profile",
  "Get authenticated hub profile/resume. Includes slug + pageUrl when vanity URL is set (workix.co/{slug}).",
  {},
...
server.tool(
  ...
  `Update performer profile (developers, designers, AND bloggers/creators/influencers). Encourage a public card with links + openTo. Claim a free vanity slug for workix.co/{slug} via slug (409 if taken by a project or another performer). ${HUB_FIELD_GUIDE}`,
  {
    ...
    slug: z
      ...
      .optional()
      .describe(
        'Vanity URL workix.co/{slug} when free. Example: "devstorm". Empty string "" clears it. Shared namespace with project slugs — hub returns 409 if taken.',
      ),
    headline: z
      .string()
      .max(200)
