...
export async function refreshJobs(opts?: {
  ...
  if (platforms?.includes("x")) {
    const r = await cached("x", { keywords: opts?.keywords }, force, () =>
      fetchXLeads({ keywords: opts?.keywords }),
    );
    collected.push(...r.jobs);
    note("x", r);
  ...
  if (platforms?.includes("profi") || servicesDefault) {
    ...
    const r = await cached("profi", { keywords: opts?.keywords }, force, () =>
      fetchProfiJobs({ keywords: opts?.keywords }),
    );
    collected.push(...r.jobs);
    note("profi", r);
    if (r.error && !softError(r.error)) errors.push(r.error);
  }
  if (platforms?.includes("avito") || servicesDefault) {
    const { fetchAvitoJobs } = await import("./adapters/avito.js");
    const r = await cached("avito", { keywords: opts?.keywords }, force, () =>
      fetchAvitoJobs({ keywords: opts?.keywords }),
    );
    collected.push(...r.jobs);
    note("avito", r);
    if (r.error && !softError(r.error)) errors.push(r.error);
