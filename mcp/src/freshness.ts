 ...
 */
function redirectedUpwards(requested: string, finalUrl?: string): boolean {
  ...
  try {
    ...
    if (aPath === bPath || aPath.length <= 1) return false;
 ...
 */
function goneMarker(html: string): string | undefined {
  const body = html
    .replace(/<script[\s\S]*?<\/script>/gi, " ")
    .replace(/<template[\s\S]*?<\/template>/gi, " ")
    .replace(/<noscript[\s\S]*?<\/noscript>/gi, " ");
  for (const rx of GONE_MARKERS) {
    if (rx.test(body)) return rx.source;
  }
  return undefined;
}
...
const rootTitles = new Map<string, string | undefined>();

async function rootTitle(origin: string): Promise<string | undefined> {
  if (rootTitles.has(origin)) return rootTitles.get(origin);
  let title: string | undefined;
  try {
    const res = await fetchText(origin, { proxy: false, timeoutMs: 15000 });
    if (res.ok) title = pageTitle(res.text);
  } catch {
...
export async function checkLink(url: string): Promise<LivenessResult> {
  let res;
  try {
    res = await fetchText(url, { proxy: false, timeoutMs: 15000 });
  } catch (e) {
    return { url, state: "unknown", reason: (e as Error)?.message };
  }
  ...
  if (marker) return { url, state: "gone", reason: `маркер: ${marker}` };
...
export async function checkJobsAlive(
  jobs: Job[],
