...
export function ttlMinutesFor(source: string): number {
 ...
 */
export function cacheKey(source: string, params?: unknown): string {
  const normalized = JSON.stringify(params ?? {}, (_k, v) =>
    Array.isArray(v) ? [...v].map(String).sort() : v,
  );
...
export function readCache(
  ...
  const row = db()
    .prepare(
      "SELECT fetched_at, expires_at, payload FROM fetch_cache WHERE key = ?",
    )
    .get(cacheKey(source, params)) as
    | { fetched_at: string; expires_at: string; payload: string }
    | undefined;
  if (!row) return undefined;
  if (new Date(row.expires_at).getTime() <= Date.now()) return undefined;

  try {
    const jobs = JSON.parse(row.payload) as Job[];
    return {
      jobs,
...
export function writeCache(source: string, params: unknown, jobs: Job[]): void {
  const ttl = ttlMinutesFor(source);
  if (ttl <= 0) return;
  ...
  db()
    .prepare(
      `INSERT INTO fetch_cache (key, source, fetched_at, expires_at, items, payload)
       VALUES (?, ?, ?, ?, ?, ?)
...
export function cacheStatus(): {
  ...
  const conn = db();
  const entries = (
    conn.prepare("SELECT COUNT(*) AS c FROM fetch_cache").get() as { c: number }
  ).c;
  const fresh = (
    conn
  ...
  const rows = conn
    .prepare(
      `SELECT source, COUNT(*) AS entries, SUM(items) AS items, MAX(fetched_at) AS newest
  ...
  return {
    ...
    sources: rows.map((r) => ({
      ...
      items: r.items || 0,
      ageMinutes: Math.round(
        (Date.now() - new Date(r.newest).getTime()) / 60000,
      ),
