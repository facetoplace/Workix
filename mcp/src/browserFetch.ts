...
> {
  ...
  for (const spec of candidates) {
    try {
      ...
      const p = (mod.default || mod) as { launch?: (o: unknown) => Promise<BrowserLike>; connect?: (o: unknown) => Promise<BrowserLike> };
      if (typeof p?.launch === "function" && typeof p?.connect === "function") {
        return p as { launch: (o: unknown) => Promise<BrowserLike>; connect: (o: unknown) => Promise<BrowserLike> };
      }
    } catch { /* try next */ }
  }
  return undefined;
...
async function acquire(
  profile: string,
  headful: boolean,
): Promise<{ browser: BrowserLike; owned: boolean } | { error: string }> {
  const puppeteer = await loadPuppeteer();
  if (!puppeteer) return { error: "puppeteer-core unavailable — cd mcp && npm install puppeteer-core" };
  ...
  if (!executablePath) return { error: "Chrome not found — set CHROME_PATH" };
  try {
...
export async function withProfilePage<T>(
  profile: string,
  url: string,
  extract: (...a: unknown[]) => unknown,
  opts?: { waitMs?: number; timeoutMs?: number; scrolls?: number; headful?: boolean },
): Promise<{ data?: T; error?: string }> {
  const acquired = await acquire(profile, Boolean(opts?.headful));
  if ("error" in acquired) return { error: acquired.error };
  ...
  try {
    ...
    const data = (await page.evaluate(extract)) as T;
    return { data };
  } catch (e) {
    return { error: (e as Error)?.message || String(e) };
  } finally {
