...
export async function runJobState(args: {
  ...
  const applied = applyAttempts.filter((o) =>
    ["sent", "ok", "reply"].includes(o.status),
  );
  ...
  if (appliedAnywhere && s.hubShare && hubApplication) next.push("nothing outstanding");
...
async function lookupHubApplication(opts: {
  ...
  if (opts.check === false || !opts.url) return null;
  if (!process.env.WORKIX_AGENT_KEY && !process.env.WORKIX_API_KEY) return null;
  try {
    const res = (await hubListApplications({ url: opts.url, limit: 1 })) as {
      ok: boolean;
