...
const PLATFORM_TIPS: Record<string, string[]> = {
  ...
  avito: [
    ...
    "Avito Работа: на карточке вакансии — «Откликнуться» / чат с работодателем.",
    "Вставь proposal_text в сообщение / отклик; не жать Отправить без «ок».",
  ],
  youdo: [
...
export async function runPrepareBrowserApply(args: {
  ...
  if (!proposal_text) {
    return {
      error:
        "Нет текста отклика. Сначала workix_draft_proposal (агент пишет текст) и mode=save.",
    };
  }
