 ...
 * people type.
 */
export function parseTgQueryTerms(query: string): string[] {
...
export async function runTgSearch(args: {
  ...
  for (const chat of chatsToSearch) {
    const byId = new Map<string, TgMessageHit>();
    const errors: string[] = [];
    const matchedTerms: string[] = [];
    for (const term of terms) {
      try {
        const hits = await searchChat(chat, term, perChat, sinceInfo.since);
        if (hits.length) matchedTerms.push(term || "(recent)");
        perTerm[term || "(recent)"] =
  ...
  return {
    ...
    ...(chats.length > chatsToSearch.length
      ? {
          ...
          chats_note:
            "Raise max_chats (or pass chats:[…]) to cover the rest of telegram-channels.json.",
        }
      : {}),
...
export async function runTgSend(args: {
  ...
  if (!to) return { ok: false, error: "to required (@username, t.me link, or numeric id)" };
