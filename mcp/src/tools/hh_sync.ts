 ...
 */
export function outreachStatusForNegotiation(item: NegotiationItem): OutreachStatus {
  const state = String(item.raw_state || "").toUpperCase();
  if (state === "DISCARD_BY_APPLICANT") return "skip";
  if (item.rejected || state.startsWith("DISCARD") || state === "RESPONSE_REJECTED") {
    return "blocked";
  }
  if (item.invited || REPLY_STATES.has(state)) return "reply";
  if (item.unread_messages || item.question_pending) return "reply";
  return "sent";
}

function vacancyIdOf(item: NegotiationItem): string | undefined {
...
export async function runHhSyncOutreach(args: {
  ...
  const created: Array<{ id: string; status: string; vacancy: string }> = [];
  const updated: Array<{ id: string; from: string; to: string; vacancy: string }> = [];
  const unchanged: string[] = [];
  const skipped: Array<{ vacancy: string; reason: string }> = [];
  ...
  for (const item of neg.items) {
    ...
    if (!args.dry_run) {
      logOutreach({
        ...
        channel: "hh",
        contact,
        text: `[импорт из hh negotiations] Отклик на «${
          item.vacancy || vacancyId
        }» отправлен ${String(at).slice(0, 10)}. Текст сопроводительного письма в этом канале не сохранён.`,
        project: item.vacancy,
        url: item.vacancy_url,
        jobId: job?.id,
        note: `hh: ${item.state || status}${
          item.messages ? `, сообщений: ${item.messages}` : ""
        }`,
      });
    }
