...
interface Topic {
  ...
  applicantQuestionState?: boolean;
  conversationMessagesCount?: number;
  lastModified?: string;
  creationTime?: string;
  archived?: boolean;
  declinedByApplicant?: boolean;
...
function isRejection(state?: string): boolean {
  return Boolean(state && state.startsWith("DISCARD"));
}

function money(v?: ShortVacancy["compensation"]): string | undefined {
...
export async function runHhNegotiations(args: {
  ...
  const topics: Topic[] = [];
  const vacancies = new Map<number, ShortVacancy>();
  let total = 0;
  let counters: unknown = null;
  let lastError: string | undefined;

  for (let page = 0; page < maxPages; page++) {
    const { state, error } = await fetchHhState(
    ...
    const neg = state.applicantNegotiations as
      | { topicList?: Topic[]; total?: number; pageCount?: number }
      | undefined;
    if (page === 0) {
      total = neg?.total ?? 0;
    ...
    const short = state.vacanciesShort as
      | { vacanciesList?: ShortVacancy[] }
      | undefined;
    for (const v of short?.vacanciesList ?? []) {
      if (v.vacancyId != null) vacancies.set(v.vacancyId, v);
    }

    const list = neg?.topicList ?? [];
    topics.push(...list);
    if (!list.length || page + 1 >= (neg?.pageCount ?? 1)) break;
  }
  ...
  if (args.only_new) items = items.filter((i) => i.needs_reply);
  if (filter === "invitation") items = items.filter((i) => i.invited);
  else if (filter === "rejected") items = items.filter((i) => i.rejected);
