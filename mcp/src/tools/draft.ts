...
export async function runDraftProposal(args: {
  ...
  return {
    ...
    constraints: [
      ...
      "Упомяни возможные трудности и почему выбрать именно исполнителя",
    ...
    example_style:
      "Короткий деловой тон. Сначала понимание задачи, затем релевантный опыт, риск/сложность, оффер по сроку и цене.",
    instruction_for_agent:
      "Напиши готовый текст отклика по brief выше, покажи пользователю. После одобрения сохрани через workix_draft_proposal mode=save.",
  };
}
