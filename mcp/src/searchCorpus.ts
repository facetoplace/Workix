...
export const applyNorm = (s: string): string =>
...
export const RESUME_RE =
  /#резюме|#resume|#ищу\b|ищу работу|ищу проект|рассмотрю предложени|открыт к предложени|обо мне[:\s]|мои навыки|мой стек|готов присоединиться к стартапу|\[\s*for\s*hire\s*\]|\bfor hire\b|\bopen to work\b|available for (hire|work|new projects|freelance)|seeking (a\s+)?(new\s+)?(remote\s+)?(role|position|opportunit|job)/i;
 ...
 */
export function advertiserKey(title: string): string | null {
  const t = String(title || "");
  let m = t.match(/@\s*([\p{L}0-9][\p{L}0-9 .&'\-]{1,28})\s*$/u);
  if (m) return applyNorm(m[1]);
...
export function buildAppliedIndex(
  ...
  corpus: Array<{ title: string; description?: string }>,
): AppliedIndex {
  ...
  for (const w of wordCandidates) {
    let df = 0;
    for (const h of haystacks) if (h.includes(` ${w} `)) { if (++df > DF_MAX) break; }
    if (df <= DF_MAX) keys.add(w);
  }
  return { urls, keys: [...keys] };
}

export function matchApplied(
  ...
  for (const k of idx.keys) if (hay.includes(` ${k} `) || hay.includes(` ${k}`)) return "name";
  return null;
}
...
export function searchCorpus(
  corpus: Job[],
  outreach: OutreachLike[],
  opts: CorpusSearchOpts,
): { total: number; duplicates_collapsed: number; applied_count: number; open_count: number; results: CorpusResult[] } {
  ...
  const byKey = new Map<string, { also: string[] }>();
  const deduped: Array<(typeof ranked)[number] & { also: string[] }> = [];
  let collapsed = 0;
  for (const x of ranked) {
    const key = dupKey(x.j);
    const seen = byKey.get(key);
    if (seen) { collapsed++; seen.also.push(x.j.link); continue; }
    const entry = { also: [] as string[] };
    byKey.set(key, entry);
